'FMBM BM Refresh Version 1.1 date 12/22/2021
...
Sub Main
	...
	DIM userRoleFileAtShare, userRoleFileAtShareLM, userRoleFileLM, a, b, c, LogFile, TypeKey, FMBMLog, FMBMFileSize
	...
	If Err.Number <> 0 Then
		ErrorBuffer= ErrorBuffer & Now() & ":  BM Refresh - " & loggedinUser & ": Error while checking for FMBM folder: " & Err.Description & vbCrLf
		Err.Clear
	End If
	...
	'If file exists read shareLocation, roleQuery and userRoleFile from the file
		...
		do while not FMBMLoggedInUserFile.AtEndOfStream
			...
			Else 
				...
				ErrorBuffer = ErrorBuffer  & Now() & ":  BM Refresh - " & loggedinUser & " <--Logged in User"  & vbCrLf
			End If
		loop
	Else
		Wscript.Quit
	...
	If Err.Number <> 0 Then
		ErrorBuffer= ErrorBuffer & Now() & ":  BM Refresh - " & loggedinUser & ": Error while checking for loggid in User :" & Err.Description & vbCrLf
		Err.Clear
	End If
	...
	'If file exists read shareLocation, roleQuery and userRoleFile from the file
		...
		LogFile.WriteLine(Now()  & ":  BM Refresh - " & loggedinUser &  ": FMBM property file exist " & FMBMFile)
		iCounter = 1
	...
	Else 
		...
		LogFile.WriteLine(Now() &  ":  BM Refresh - " & loggedinUser & ": Exiting code as FMBM property file do not exist")
		Wscript.Quit
	End If
	...
	If fso.FileExists(userRoleFileAtShare) Then 
		...
		LogFile.WriteLine(Now() & ":  BM Refresh - " & loggedinUser & ": User Role file at share exists and is " & userRoleFileAtShare)
		...
		If(b.size = 0 or c.size = 0) Then
			LogFile.WriteLine(Now() & ":  BM Refresh - " & loggedinUser &  ": Exiting code as file size is 0")
			LogFile.WriteLine(Now() & ":  BM Refresh - " & loggedinUser &  ": Share " & userRoleFileAtShareLM & " size = " & b.size)
			LogFile.WriteLine(Now() & ":  BM Refresh - " & loggedinUser &  ": LP " & userRoleFileLM & " size = " & c.size)
			Wscript.Quit
		End If
		...
		If(userRoleFileAtShareLM<userRoleFileLM) Then 
			...
			If Err.Number <> 0 Then
				LogFile.WriteLine(Now() & ":  BM Refresh - " & loggedinUser & ": Error while copying file from Launchpad folder to roleshare: " & Err.Description)
				Err.Clear
			End If	
			LogFile.WriteLine(Now() & ":  BM Refresh - " & loggedinUser & ": File at share is old, copying from Launchpad folder")
		End If
