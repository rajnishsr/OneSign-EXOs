'FMBM Lock Version 1.2 date 12/22/2021
...
Sub Main
	...
	If Err.Number <> 0 Then
		ErrorBuffer= ErrorBuffer & Now() & ":  Lock - " & loggedinUser & ": Error while checking for FMBM folder: " & Err.Description & vbCrLf
		Err.Clear
	End If
	...
	'If file exists read shareLocation, roleQuery and userRoleFile from the file
		...
		do while not FMBMLoggedInUserFile.AtEndOfStream
			...
			Else 
				...
				ErrorBuffer = ErrorBuffer  & Now() & ":  Lock - " & loggedinUser & " <--Logged in User"  & vbCrLf
			End If
		loop
	End If
	
	If Err.Number <> 0 Then
		ErrorBuffer= ErrorBuffer & Now() & ":  Lock - " & loggedinUser & ": Error while checking for loggid in User :" & Err.Description & vbCrLf
		Err.Clear
	End If
	...
	'If file exists read shareLocation, roleQuery and userRoleFile from the file
		...
		LogFile.WriteLine(Now()  & ":  Lock - " & loggedinUser &  ": FMBM property file exist " & FMBMFile)
		iCounter = 1
	...
	Else 
		...
		LogFile.WriteLine(Now() &  ":  Lock - " & loggedinUser & ": Exiting code as FMBM property file do not exist")
		Wscript.Quit
	End If
	...
	If fso.FileExists(userRoleFileAtShare) Then 
		...
		LogFile.WriteLine(Now() & ":  Lock - " & loggedinUser & ": User Role file at share exists and is " & userRoleFileAtShare)
		...
		If(b.size = 0 or c.size = 0) Then
			LogFile.WriteLine(Now() & ":  Lock - " & loggedinUser &  ": Exiting code as file size is 0")
			LogFile.WriteLine(Now() & ":  Lock - " & loggedinUser &  ": Share " & userRoleFileAtShareLM & " size = " & b.size)
			LogFile.WriteLine(Now() & ":  Lock - " & loggedinUser &  ": LP " & userRoleFileLM & " size = " & c.size)
			Wscript.Quit
		End If
		...
		If(userRoleFileAtShareLM<userRoleFileLM) Then 
			...
			If Err.Number <> 0 Then
				LogFile.WriteLine(Now() & ":  Lock - " & loggedinUser & ": Error while copying file from Launchpad folder to roleshare: " & Err.Description)
				Err.Clear
			End If	
			LogFile.WriteLine(Now() & ":  Lock - " & loggedinUser & ": File at share is old, copying from Launchpad folder")
		End If
